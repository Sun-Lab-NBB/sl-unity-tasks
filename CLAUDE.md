...
- `delete_asset` is bounded by `DeleteAllowedPrefixes` and `DeleteProtectedPaths` at the top of `McpBridge.cs`, and
  scene deletion goes through `delete_task` so the per-scene companion cascade is never bypassed. Segment ownership
  resolves by the longest matching template basename, so `SSO_Merging` never sweeps `SSO_Merging_Base` segments, and
  any future per-scene companion joins `McpBridge.TryDeleteScenePerSceneCompanions` in the same change.
...
- **Schema mirror**: `TaskTemplate`, `Cue`, `TrialStructure`, and `VREnvironment` mirror the Python `YamlConfig`
  ...
  codes, the trial-name pattern, the `trigger_type` literal set, per-trial `cue_sequence` uniqueness, a positive
  `occupancy_duration_ms`, and the transition targets and probability sum. Per-mode geometric zone validation lives in
  the shared-assets Python `TaskTemplate`, not here.
...
- **Zone composition**: `StimulusTriggerZone` dispatches on a `TriggerMode` enum that `CreateTask` sets from the
  ...
  generic `occupancyMet` signal and the parent applies the per-mode rule. `ResetZone` caches the `StimulusTriggerZone`,
  `OccupancyZone`, and `OccupancyGuidanceZone` instances at `Start`, so a standalone `IResettable` needs its own
  `FindObjectsByType` line there. See `/zone-prefabs`.
...
- **Hand-authored vs generated assets**: `Padding.prefab`, `ResetZone.prefab`, `StimulusTriggerZone.prefab`,
  ...
  `ExperimentTemplate.unity` is generated by `CreateTask`. All nine hand-authored assets are protected by
...
- **Segment regeneration**: `CreateTask.CleanGeneratedSegments` deletes every segment prefab the template owns before
  each pass, so trial-parameter edits never leave stale geometry under an unchanged `TemplateName_TrialName` filename.
  Cue prefabs and materials are preserved across runs, and only segments are always-regenerate.
...
Preserve the `IResettable` contract on any zone holding per-lap state, and register a new implementer in `ResetZone`.
