...
namespace Gimbl
{
    ...
    /// <remarks>
    /// Widths are sized to fit the longest expected line, illustratively the Camera Mapping row. The row's label reads
    /// <c>"Monitor N (x, y)"</c> followed by a camera-name Popup (its entries are "None" or a scene camera name). The
    /// ~170 px monitor label, an unmeasured ~95 px dropdown estimate, and ~15 px of MainBox padding together add up to
    /// roughly the 280 px <see cref="MainBoxStyle"/> width.
    /// </remarks>
